...
import { MessageType, OptionsType } from './type'
...
>(
  ...
  const loadData = async (page = pageIndex.value) => {
    loading.value = true
    try {
      preRequest?.()
      const {
      ...
      GET_DATA_IF_FAILED && createErrorMessage(GET_DATA_IF_FAILED)
  ...
  const exportFile = async () => {
    ...
    try {
      ...
      EXPORT_DATA_IF_FAILED && createErrorMessage(EXPORT_DATA_IF_FAILED)
      options?.exportError?.()
    }
  }
